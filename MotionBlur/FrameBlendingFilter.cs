...
namespace Kino
{
    public partial class Motion
    {
        ...
        public class FrameBlendingFilter
        {
            ...
            public FrameBlendingFilter(Shader shader, int frameListCount)
            {
                ...
                _frameList = new Frame[frameListCount];
            }
            ...
            )
            {
                ...
                */
                ...
                {
                    var frame = GetFrameRelative(-1 - i);

                    _material.SetTexture($"_History{ 1 + i }LumaTex", frame.lumaTexture);

                    _material.SetTexture($"_History{ 1 + i }ChromaTex", frame.chromaTexture);

                    _material.SetFloat($"_History{ 1 + i }Weight", frame.CalculateWeight(strength, t));
                }
